...
(defmacro CLEAN [] (reset! DISPATCHMAP {}))
...
(defmacro pdf [-sym args & more] 
  (let [[_ -ns -s] (re-find #"(.+)[\/](.+)" (str -sym))
        ...
        compiled (cons 'fn (map 
                    ...
                    (get @DISPATCHMAP (qualified sym NS))))
...
;  `(clojure.pprint/pprint (~'macroexpand (quote ~form))))

;(reset! DISPATCHMAP {})
(macroexpand '(defpdf asdf))
;(macroexpand  '(pdf fish [a b] :ab))
